using System;
using System.Collections.Generic;
using System.IO;
using System.Net;
using System.Net.Http;
using System.Net.Sockets;
using System.Text;
using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using CarRentalLogicServer.APIConsumer;
using CarRentalLogicServer.Models;

namespace CarRentalLogicServer.ClientServerHost
{
    public class Server
    {
        private ICarService CarService;

        public Server()
        {
            CarService = new WebCarService();

            /*Console.WriteLine("\nHit enter to continue...");
            Console.Read();*/
        }

        public async Task Listen()
        {
            TcpListener server = null;
            try
            {
                // Set the TcpListener on port 13000.
                Int32 port = 13000;
                IPAddress localAddr = IPAddress.Parse("127.0.0.1");

                // TcpListener server = new TcpListener(port);
                server = new TcpListener(localAddr, port);

                // Start listening for client requests.
                server.Start();

                // Buffer for reading data
                Byte[] bytes = new Byte[256];
                String incommingMessage;

                // Enter the listening loop.
                while (true)
                {
                    Console.Write("Waiting for a connection... ");

                    // Perform a blocking call to accept requests.
                    // You could also use server.AcceptSocket() here.
                    TcpClient client = server.AcceptTcpClient();
                    Console.WriteLine("Connected!");

                    incommingMessage = null;

                    // Get a stream object for reading and writing
                    NetworkStream stream = client.GetStream();

                    int i;

                    // Loop to receive all the data sent by the client.
                    while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                    {
                        // Translate data bytes to a ASCII string.
                        incommingMessage = System.Text.Encoding.ASCII.GetString(bytes, 0, i);
                        Console.WriteLine("Received: {0}", incommingMessage);

                        string outgoingMessage;


                        switch (incommingMessage)
                        {
                            case "GetCars":
                                var carsAsync = await CarService.GetCarsAsync();
                                outgoingMessage = JsonSerializer.Serialize(carsAsync);
                                break;
                            default:

                                Car incomingCar;
                                try
                                {
                                    incomingCar = JsonSerializer.Deserialize<Car>(incommingMessage);
                                    if (incomingCar == null)
                                    {
                                        incomingCar = new Car(-2, "null car", "null car");
                                    }
                                }
                                catch (Exception e)
                                {
                                    incomingCar = new Car(-2, "null car", "null car");
                                    Console.WriteLine(e);
                                }
                                
                                if (incomingCar.Id != -2)
                                {
                                    var createdCar = await CarService.CreateCarAsync(incomingCar);
                                    outgoingMessage = JsonSerializer.Serialize(createdCar);
                                }
                                else
                                {
                                    outgoingMessage = "Unknown command";
                                }

                                break;
                        }


                        // Process the data sent by the client.
                        // outgoingMessage = incommingMessage.ToUpper();

                        byte[] msg = System.Text.Encoding.ASCII.GetBytes(outgoingMessage);

                        // Send back a response.
                        stream.Write(msg, 0, msg.Length);
                        Console.WriteLine("Sent: {0}", outgoingMessage);
                    }

                    // Shutdown and end connection
                    client.Close();
                }
            }
            catch (SocketException e)
            {
                Console.WriteLine("SocketException: {0}", e);
            }
            finally
            {
                // Stop listening for new clients.
                server.Stop();
            }
        }
    }
}